...
def inner_circle(src, pt, size = 80):
    """
    領域の内接円中心を求める
    Parameters
    ----------
...
def process(filename):
    ...
    cv2.imshow('pic read', src)  #user
    print('画像読み込み')
    cv2.waitKey(0)          #user

    #src = cv2.flip(src, 1)      #user

    #cv2.imshow('flip', src)  #user
    #print('左右裏返し')
    #cv2.waitKey(0)          #user
    ...
    ret, threshold = cv2.threshold(opening, 30, 255, cv2.THRESH_BINARY_INV)
    ...
    params.minArea = 80 * 80 * math.pi / 4
    params.maxArea = 80 * 80 * math.pi
    ...
    # ブロブを赤丸で囲む
    blank = np.zeros((1, 1))  
    blobs = cv2.drawKeypoints(src, keypoints, blank, (255, 0, 255), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS) 
    ...
    for k in kps:
        ...
        cv2.line(blobs, pt1, pt2, (255,0,255), 1)
